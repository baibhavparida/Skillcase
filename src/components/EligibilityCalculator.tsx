...
type Qualification = "BSc Nursing" | "GNM Nursing" | "MSc / Post Basic" | "Other";
type Experience = "Fresher" | "Under 1 Year" | "1-2 Years" | "2+ Years";
type GermanLevel = "Not Started" | "A1 / A2" | "B1 / B2";

const qualOptions: { value: Qualification; label: string; sub: string }[] = [
...
const expOptions: { value: Experience; label: string }[] = [
...
const germanOptions: { value: GermanLevel; label: string }[] = [
...
export default function EligibilityCalculator() {
  const [qualification, setQualification] = useState<Qualification>("BSc Nursing");
  const [experience, setExperience] = useState<Experience>("1-2 Years");
  const [germanLevel, setGermanLevel] = useState<GermanLevel>("Not Started");
  ...
  const results = useMemo(() => {
    ...
    if (qualification === "MSc / Post Basic") { baseSalaryEuro += 300; baseSalaryINR += 30000; jobMatches += 20; }
    else if (qualification === "GNM Nursing") { baseSalaryEuro -= 100; baseSalaryINR -= 10000; jobMatches -= 10; }
    else if (qualification === "Other") { baseSalaryEuro -= 200; baseSalaryINR -= 20000; timelineMonths += 2; jobMatches -= 30; }

    if (experience === "2+ Years") { baseSalaryEuro += 400; baseSalaryINR += 35000; jobMatches += 35; }
    else if (experience === "Fresher") { baseSalaryEuro -= 200; baseSalaryINR -= 20000; jobMatches -= 20; }

    if (germanLevel === "B1 / B2") { timelineMonths -= 6; jobMatches += 40; baseSalaryEuro += 150; }
    else if (germanLevel === "A1 / A2") { timelineMonths -= 3; jobMatches += 15; }
    ...
    <div className="ec-shell">
      ...
      <div className="ec-results">
        <div className="ec-salary-block">
          ...
          <strong className="ec-salary-value">€{results.euroLow} – €{results.euroHigh}</strong>
        ...
        <div className="ec-stats-row">
          ...
          <div className="ec-stat">
            <span className="ec-stat-value ec-stat-value--cyan">{results.jobs}+</span>
